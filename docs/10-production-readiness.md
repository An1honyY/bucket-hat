...
- **App icon** (2026-07-21 redesign — see DECISIONS.md): an NZ-outdoors
  ...
  color ever needs to change, so they stay a matched set.

  Two sizing rules, both learned the hard way on 2026-07-31 and both easy to
  breach by re-exporting from the source at its natural crop:

  - **The adaptive foreground must fit Android's safe zone**, a circle of
    only 66% of the canvas diameter. Launchers mask to circles, squircles
    and rounded squares, and anything outside that circle can be cropped.
    The hat previously spanned 66% of the *width*, so its brim — which is
    below centre, where the circle is already narrowing — was being clipped.
    It now spans 53%, putting the furthest opaque pixel at 88% of the safe
    radius. Check this numerically after any change; it is not obvious by eye
    from a square preview.
  - **`favicon.png` has no background.** A browser tab sits on the browser's
    own chrome, which is light or dark depending on the user's theme, so a
    baked-in navy square reads as a misaligned box rather than as the mark.
    It's the hat cropped tight, on transparency — generated by flood-filling
    the navy inward from the corners rather than by matching that colour
    globally, since the hat's near-black outline is numerically close to it
    and a global match punches through the linework. Avoids literal
