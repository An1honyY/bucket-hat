...
export default function NavIcon({ kind, size = 22, color }: Props) {
  ...
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      ...
      {kind === "settings" && (
        // Explicit fill="none" on both shapes. Every other glyph here gets it
        // via the shared `stroke` spread; these two were relying on inheriting
        // it from the root <Svg fill="none">, which holds on web but not in
        // react-native-svg, where an unset fill paints solid black. That is
        // why the cog read as a black blob on the dark header on device while
        // measuring as near-white in the browser.
        <>
