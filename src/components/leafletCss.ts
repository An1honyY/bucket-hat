// Vendored verbatim from node_modules/leaflet/dist/leaflet.css (v1.9.4) at
// build time — LocationPickerMap.web.tsx injects this as an inline <style>
// tag rather than `import "leaflet/dist/leaflet.css"` directly, since this
// project has no established precedent for Metro web bundling raw CSS
// imports (unlike the wasm-as-asset fix in metro.config.js, which is a
// proven pattern for a different problem) and no CDN dependency should be
// needed just to render a picker that otherwise works fully offline.
// Leaflet ships no marker-image assets here either — LocationPickerMap.web
// uses an inline SVG L.divIcon instead, so this stylesheet alone is
// sufficient (no additional asset loading of any kind).
